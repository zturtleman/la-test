...
static int GLimp_SetMode(int mode, qboolean fullscreen, qboolean noborder, qboolean fixedFunction)
{
	...
	const GLubyte * (APIENTRYP qglGetString) (GLenum name) = NULL;
	void (APIENTRYP qglClearColor) (GLclampf red, GLclampf green,
									GLclampf blue, GLclampf alpha) = NULL;
	void (APIENTRYP qglClear) (GLbitfield mask) = NULL;
	const char *glstring;
	...
	for ( type = 0; type < numContexts; type++ ) {
		...
		for (i = 0; i < 16; i++)
		{
			...
			if ( qglGetString && contexts[type].profileMask == SDL_GL_CONTEXT_PROFILE_CORE )
			{
				const char *renderer;
...
success:
	...
	R_InitExtensions( fixedFunction );
